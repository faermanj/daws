...
- project - Code for the application's Lambda function.
...
```bash
sam logs -n ProjectFunction --stack-name "project" --tail
```
...
# Get API URL
URL=$(aws cloudformation describe-stacks --stack-name project --query "Stacks[0].Outputs[?OutputKey=='ProjectApi'].OutputValue" --output text)
...
curl -s "$URL/?action=read&pet_id=2" | jq 

# Delete
curl -s "$URL/?action=delete&pet_id=4"
```